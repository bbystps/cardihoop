...
EXPECTED_TOTAL_SAMPLES = TARGET_FS * WINDOW_SECONDS
...
# HEART RATE HELPERS
# =========================================================
def estimate_heart_rate_bpm(samples: np.ndarray, fs: float) -> Tuple[Optional[int], List[int]]:
    """
    Simple ECG HR estimator:
    baseline removal -> smoothing -> derivative energy -> envelope -> peak detection
    Returns:
        (heart_rate_bpm, detected_peak_indices)
    """
    if samples is None or len(samples) < int(fs * 2) or fs <= 0:
        return None, []

    x = samples.astype(np.float32, copy=False)

    # 1) Baseline removal (~0.6s moving average subtraction)
    win_hp = max(1, int(fs * 0.6))
    hp = np.zeros_like(x, dtype=np.float32)

    run_sum = 0.0
    for i in range(len(x)):
        run_sum += float(x[i])
        if i >= win_hp:
            run_sum -= float(x[i - win_hp])

        mean_v = run_sum / float(min(i + 1, win_hp))
        hp[i] = x[i] - mean_v

    # 2) Light smoothing (~0.05s)
    win_lp = max(1, int(fs * 0.05))
    if win_lp > 1:
        kernel_lp = np.ones(win_lp, dtype=np.float32) / float(win_lp)
        lp = np.convolve(hp, kernel_lp, mode="same").astype(np.float32)
    else:
        lp = hp.copy()

    # 3) Derivative energy
    energy = np.zeros_like(lp, dtype=np.float32)
    if len(lp) > 1:
        d = np.diff(lp)
        energy[1:] = d * d

    # 4) Envelope (~0.12s)
    win_env = max(1, int(fs * 0.12))
    if win_env > 1:
        kernel_env = np.ones(win_env, dtype=np.float32) / float(win_env)
        env = np.convolve(energy, kernel_env, mode="same").astype(np.float32)
    else:
        env = energy.copy()

    mean_env = float(np.mean(env))
    std_env = float(np.std(env))
    thresh = mean_env + (2.0 * std_env)

    refractory = max(1, int(fs * 0.28))
    refine_win = max(1, int(fs * 0.06))
    ...
    for i in range(1, len(env) - 1):
        ...
        is_local_max = (env[i] > env[i - 1]) and (env[i] >= env[i + 1])
        if not is_local_max:
            continue
        ...
        start = max(0, i - refine_win)
        end = min(len(lp) - 1, i + refine_win)
    ...
    rr = np.diff(np.asarray(peaks, dtype=np.float32)) / float(fs)
    rr = rr[rr > 0]
    ...
    bpm = 60.0 / rr_med
    bpm = int(round(max(30.0, min(220.0, bpm))))

    return bpm, peaks


def get_rate_label(heart_rate_bpm: Optional[int]) -> Optional[str]:
...
def build_final_labels(ml_label: Optional[str], heart_rate_bpm: Optional[int]) -> dict:
    """
    Returns:
        rhythm_label: NORMAL / AFIB / UNKNOWN
        rate_label: BRADYCARDIA / NORMAL / TACHYCARDIA / None
        final_label: NORMAL / ABNORMAL / UNKNOWN
    """
    rhythm_raw = (ml_label or "").strip().upper()
    rate_label = get_rate_label(heart_rate_bpm)
    ...
    if rhythm_raw == "NORMAL":
        final_label = "NORMAL" if rate_label == "NORMAL" else "ABNORMAL"
        return {
            "rhythm_label": "NORMAL",
            "rate_label": rate_label,
...
def infer_json_file(json_path: str) -> dict:
    ...
    Returns ML decision + signal used for analysis.
    """
    ...
    if vote:
        ...
        if len(top) == 1:
            ml_label = top[0]
            ml_conf = max(
                (w["conf"] for w in window_best if w["label"] == ml_label),
                default=window_best_sorted[0]["conf"],
            )
        else:
            ml_label = window_best_sorted[0]["label"]
            ml_conf = window_best_sorted[0]["conf"]
    ...
    return {
        ...
        "analysis_fs": float(effective_fs),
        "preprocess": {
    ...
    try:
        ...
        if payload_text.startswith("SCAN_END"):
            ...
            try:
                ...
                analysis_samples = infer.get("analysis_samples")
                analysis_fs = float(infer.get("analysis_fs") or 0.0)

                heart_rate_bpm, detected_peaks = estimate_heart_rate_bpm(
                    analysis_samples,
                    analysis_fs
                )
                ...
                print(
                    f"[RESULT] rhythm={labels.get('rhythm_label')} "
                    f"rate={labels.get('rate_label')} "
                    f"final={labels.get('final_label')} "
                    f"hr={heart_rate_bpm} "
                    f"ml_conf={infer.get('ml_confidence')}"
                )
                ...
                publish_json(client, MQTT_TOPIC_RESULT, {
                    ...
                    "rhythm_label": labels.get("rhythm_label"),
                    "rate_label": labels.get("rate_label"),
                    "final_label": labels.get("final_label"),

                    "heart_rate_bpm": heart_rate_bpm,
                    "peak_count": len(detected_peaks),

                    "ml_label": infer.get("ml_label"),
                    "ml_confidence": infer.get("ml_confidence"),
                ...
                print(f"[RESULT] Published: {MQTT_TOPIC_RESULT}")
